...
-- 입학 년도가 빠른 순으로 표시하는 SQL 문장을 작성하시오. ( 단, 헤더는 "학번", "이름", "입학년도" 가 표시되도록 한다.)
SELECT 
    STUDENT_NO 학번
    , STUDENT_NAME 이름
...
-- (* 이때 올바르게 작성한 SQL문장의 결과 값이 예상과 달게 나올 수 있다. 원인이 무엇일지 생각해 볼 것)
SELECT
    PROFESSOR_NAME
    , PROFESSOR_SSN
FROM TB_PROFESSOR
...
--    , TO_NUMBER(EXTRACT (YEAR FROM TO_DATE('19' || SUBSTR(PROFESSOR_SSN, 1, 6), 'YYYYMMDD')))
    , TO_NUMBER(EXTRACT (YEAR FROM SYSDATE)) - TO_NUMBER(EXTRACT(YEAR FROM TO_DATE('19' || SUBSTR(PROFESSOR_SSN, 1, 6), 'YYYYMMDD'))) 나이
--    , TO_NUMBER('20' || SUBSTR(SYSDATE, 1, 2)) - TO_NUMBER('19' || SUBSTR(PROFESSOR_SSN, 1, 2)) 나이2
FROM TB_PROFESSOR
...
SELECT
    ...
    , STUDENT_NAME
--    , STUDENT_SSN
--    , SUBSTR(STUDENT_SSN, 1, 6)
--    , TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD')
--    , EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD'))
--    , TO_NUMBER(EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD')))
--    , TO_NUMBER(EXTRACT(YEAR FROM ENTRANCE_DATE)) - TO_NUMBER(EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD')))
--    , EXTRACT(YEAR FROM ENTRANCE_DATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD'))
FROM TB_STUDENT
WHERE EXTRACT(YEAR FROM ENTRANCE_DATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD')) > 19;


-- 6. 2020년 크리스마스는 무슨 요일인가?
SELECT TO_CHAR(TO_DATE('20201225', 'YYMMDD'), 'DAY') 크리스마스 FROM DUAL;
...
SELECT
    TO_CHAR(TO_DATE('99/10/11', 'YY/MM/DD'), 'YYYY"년" MM"월" DD"일"') DATE1
    , TO_CHAR(TO_DATE('49/10/11', 'YY/MM/DD'), 'YYYY"년" MM"월" DD"일"') DATE2
    , TO_CHAR(TO_DATE('99/10/11', 'RR/MM/DD'), 'YYYY"년" MM"월" DD"일"') DATE3
    , TO_CHAR(TO_DATE('49/10/11', 'RR/MM/DD'), 'YYYY"년" MM"월" DD"일"') DATE4
FROM DUAL;
...
-- 단, 이 때 출력 화면의 헤더는 "평점"이라고 찍히게  하고, 점수는 반올림하여 소수점 이하 한자리까지만 표시한다.
SELECT
...
-- 15. 학번이 A11213인 김고운 학생의 년도, 학기 별 평점과 년도 별 누적 평점, 총 평점을 구하는 SQL문을 작성하시오.
