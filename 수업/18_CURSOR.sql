...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('부서코드 : ' || V_DEPT.DEPT_ID || ' 부서명 : ' || V_DEPT.DEPT_TITLE || ' 지역코드 : ' || V_DEPT.LOCATION_ID);
    END LOOP;
    CLOSE C1;
END;
/
-- 프로시저 수정하고 다시 실행하면 다시 컴파일 됨
...
--@ 암시적커서
--> 암시적커서는 사용자가 직접 쿼리의 결과에 접근하지 못함 자동으로 해줌
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('성공 행 갯수 : ' || V_COUNT_ROW);
END;
/
