...
-- 원하는 행 조회
-- EMPLOYEE 테이블에서 부서코드가 'D9'인 사원 조회
...
-- DISTINCT 키워드는 SELECT절에서 딱 한번만 쓸 수 있다
-- 여러 컬럼을 묶어서 중복 제외
SELECT DISTINCT JOB_CODE, DEPT_CODE FROM EMPLOYEE; -- 2개 함께 비교해서 중복 제거

-- WHRER 절
-- 테이블에서 조건을 만족하는 값을 가진 행을 조회한다
-- 여러 개의 조건을 만족하는 행을 조회할 때 AND 또는 OR 을 사용할 수 있다

-- 부서 코드가 D6이고, 급여를 200만원보다 더 많이 받는 직원의
-- 이름, 부서 코드, 급여를 조회하시오
SELECT 
    EMP_NAME, 
    DEPT_CODE, 
...
-- 연결 연산자를 이용해서 여러 컬럼을 하나의 컬럼인 것처럼 연결 할 수 있다. (||)
-- 컬럼과 컬럼을 연결
SELECT 
...
-- 컬럼과 리터럴을 연결
SELECT
    EMP_NAME || '의 월급은 ' || SALARY || '원입니다.' VAL
FROM EMPLOYEE;
...
SELECT 
    ...
    , '재직중' 근무여부
FROM EMPLOYEE
WHERE 1=1 -- true라는 의미 안써도 된다고 함 
--WHERE 1=0 -- 1=0은 구조만 뽑을 때 씀
AND ENT_YN = 'N'; -- 아래 AND를 쭉 내려써서 위에 1=1 WHERE 옆에 놔둔거

-- EMPLOYEE 테이블에서 급여를 350만원 이상 550만원 이하 받는
...
FROM EMPLOYEE
WHERE 1=1
AND SALARY BETWEEN 3500000 AND 5500000;

-- 350만원 미만 또는 550만원 초과하는
-- 직원의 사번, 이름, 급여, 부서코드, 직급코드 조회하시오
SELECT
    EMP_ID
    , EMP_NAME
    , SALARY
...
WHERE SALARY > 5500000 OR SALARY < 3500000;
...
FROM EMPLOYEE
WHERE 1=1
AND HIRE_DATE BETWEEN '90/01/01' AND '01/01/01'; -- 그냥 이렇게 해도 됨
--AND HIRE_DATE BETWEEN TO_DATE(19910101) AND TO_DATE(20010101); -- TO_DATE 날짜비교
--AND HIRE_DATE BETWEEN TO_DATE('19910101', 'YYYYMMDD') AND TO_DATE('20010101', 'YYYYMMDD');
...
-- EMPLOYEE 테이블에서 성이 김씨가 아닌 직원의 사번, 이름, 입사일을 조회하시오
SELECT
    EMP_ID
    , EMP_NAME
    , HIRE_DATE
...
-- J2 직급의 급여 200만원 이상 받는 직원이거나 J7 직급의 직원의
-- 이름, 급여, 직급 코드 조회
SELECT
    EMP_NAME
    , SALARY
...
FROM EMPLOYEE
WHERE JOB_CODE = 'J7'
OR JOB_CODE = 'J2' AND SALARY >= 2000000; -- 이렇게하면 AND가 우선순위가 높음
-- 중요하다고 하심


-- J7 직급이거나 J2 직급인 직원들 중에서 급여가 200만원 이상인 직원의
-- 이름, 급여, 직급코드 조회
SELECT
    EMP_NAME
    , SALARY
