...
DECLARE
    DUP_EMPNO EXCEPTION;
    PRAGMA EXCEPTION_INIT(DUP_EMPNO, -00001); -- EXCEPTION 선언해주면
BEGIN
    UPDATE EMPLOYEE
    SET EMP_ID = 201
    WHERE EMP_ID = 200; -- EXCEPTION이 날 때 그 EXCEPTION이 뜸
EXCEPTION -- 그 EXCEPTION을 처리
    WHEN DUP_EMPNO
...
-- 테이블 타입의 변수 선언 값 대입 및 출력 
-- BINARY_INTEGER 로만 INDEX 선언할수있음 
-- 이렇게 선언된 테이블 타입을 가지는 변수를 선언하게되면
-- 해당 변수는 여러 데이터를 가질 수 있는 일차원 배열 형태가 된다. 
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('지급: ' || EMP_RECORD.JOB_NAME);
END;
/
