...
INSERT INTO EMPLOYEE
    VALUES( 900, '장채현', '901123-1080503', 'jang@kh.or.kr', '01055569512'
            , 'D1', 'J7', 'S3', 4300000, 0.2
            , '200', SYSDATE, NULL, DEFAULT
            ); -- DEFAULT 그냥 쓰면 기본값 들어가는거 같음
...
-- 평상시 유재식 사원을 부러워하던 방명수 사원의 급여와 보너스율을
...
-- 단체로 파업을 진행했다
-- 노옹철, 전형돈, 정중하, 하동운 사원의 급여와 버너스를
-- 유재식 사원의 급여와 보너스를 같게 변경하는 UPDATE문을 작성하시오
...
-- EMP_SALARY 테이블에서 아시아 근무지역에 근무하는 직원의 보너스 0.5로 변경하시오
UPDATE EMP_SALARY
SET BONUS = 0.5
WHERE EMP_ID IN   (
                        SELECT
...
-- FOREIGN KEY 위반 난거
UPDATE EMPLOYEE
SET DEPT_CODE = '65' -- FROEIGN KEY 제약 조건에 위배됨
WHERE DEPT_CODE = 'D6'; -- DEPARTMENT 테이블에는 65가 없기 때문

-- NOT NULL 위반
UPDATE EMPLOYEE
SET EMP_NAME = NULL -- NOT NULL 제약 조건에 위배됨
WHERE EMP_ID = 200;
...
-- 만약 WHERE절에 조건을 설정하지 않으면 모든 행 삭제
...
DISABLE CONSTRAINT SYS_C007213 CASCADE;
...
-- 비활성화된 제약조건 다시 활성화 해야함 ROLLBACK으로 해결 안됨
ALTER TABLE DEPARTMENT
ENABLE CONSTRAINT SYS_C007213;

-- TRUNCADE : 테이블의 전채행을 삭제할시 사용
-- DELETE보다 수행속도가 빠르다
...
TRUNCATE TABLE EMP_SALARY; -- TABLE EMP_SALARY가 잘렸습니다라고 뜸
SELECT * FROM EMP_SALARY; -- 값 없음 롤백 안됨
...
-- MERGE (M_TEST01 에 M_TEST02 를 병합)
-- 이건 실무에서 종종 썼다고 함
