...
-- : 테이블, 뷰, 그 밖의 데이터베이스 객체들이 저장되는 디스크상의 장소
SELECT * 
FROM DBA_USERS 
...
-- 현재 사용자에게 부여된 권한 조회
SELECT * FROM USER_TAB_PRIVS_RECD; -- SAMPLE 계정에서 확인 가능
-- 현재 사용자가 부여한 권한 조회
SELECT * FROM USER_TAB_PRIVS_MADE; -- EMPLYEE 계정에서 확인 가능
...
-- 생성된 롤 조회 (시스템 계정, SAMPLE 계정에서 조회)
